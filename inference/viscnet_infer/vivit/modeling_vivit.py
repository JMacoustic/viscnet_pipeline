...
class VivitEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pattern_backbone_name = getattr(config, "pattern_backbone", "resnet18")
        self.pattern_backbone_pretrained = bool(getattr(config, "pattern_backbone_pretrained", True))
        self.pattern_backbone_norm = str(getattr(config, "pattern_backbone_norm", "none")).lower()
        ...
        if self.pat_bool:
            ...
            self.pat_proj = nn.Linear(pat_feature_dim, config.hidden_size)
